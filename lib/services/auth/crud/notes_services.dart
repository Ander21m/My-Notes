...
// import 'package:mynotes/services/auth/crud/crudexceptions.dart';
...
//     await _ensureDbIsOpen();
//     final db = _getDatabaseOrThrow();
//     final deletedCount = await db.delete(noteTable,where: 
//     'id = ?',whereArgs: [id]);
//     if(deletedCount == 0){
...
//       _notes.removeWhere((node) => node.id == id);
//       _notes.add(note);
//       _notesStreamController.add(_notes);
...
//     final updatesCount = db.update(noteTable, {textColumn :text, isSyncedWithCloudColumn:0,},where: 'id = ?',whereArgs:  [note.id]);
...
//     return "Note, ID = $id,userId = $userId, isSyncedWithColoud = $isSyncedWithCloud";
//   }
